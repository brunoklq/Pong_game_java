...
public class GamePanel4 extends JPanel implements Runnable {
    ...
    GamePanel4() throws IOException{ // constructor for gamePane
    ...
    public void newPaddles() {
        ...
        paddle2_middle = new Paddle(800, (GAME_HEIGHT / 2) - (PADDLE_HEIGHT / 2), PADDLE_WIDTH, PADDLE_HEIGHT, 2);

        paddle1.enableControl();
        paddle1_middle.enableControl();
        paddle2.enableControl();
        paddle2_middle.enableControl();  // Added this line
    ...
    public void draw(Graphics g) { // method again to draw a graphic
        ...
        paddle1_middle.draw(g);
         // method call for whenever the cpu is used
        ball.draw(g);
        score.draw(g);
        Toolkit.getDefaultToolkit().sync();
        paddle2.draw(g);
        paddle2_middle.draw(g);
    }
    ...
    public void checkCollision() {
        ...
        if (ball.intersects(paddle2_middle)) {
            ball.xVelocity = Math.abs(ball.xVelocity);
            ball.xVelocity++;
            if (ball.yVelocity > 0) ball.yVelocity++;
            else ball.yVelocity--;
